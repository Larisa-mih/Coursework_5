...
class DBManager:
    ...
    def insert_into_table(self):
        """Функция, которая получает данные с JSON файла и заполняет таблицу"""
        self.conn = psycopg2.connect(**self.params)
        self.conn.autocommit = True
        self.cur = self.conn.cursor()
    ...
    def get_companies_and_vacancies_count(self):
        """Функция возвращает список всех компаний и количество вакансий по каждой компании."""
        self.conn = psycopg2.connect(**self.params)
        self.conn.autocommit = True
        self.cur = self.conn.cursor()
    ...
    def get_all_vacancies(self):
        """Функция возвращает список всех вакансий, зарплаты, с указанием компании и ссылки на вакансию."""
        self.conn = psycopg2.connect(**self.params)
        self.conn.autocommit = True
        self.cur = self.conn.cursor()
    ...
    def get_vacancies_with_higher_salary(self):
        ...
        self.cur = self.conn.cursor()
        self.cur.execute(
            f"""select vacancy_title,vacancy_salary from head_hunter_vacancies where vacancy_salary > (select avg(
            vacancy_salary) from head_hunter_vacancies) order by vacancy_salary""")
        data = self.cur.fetchall()
    ...
    def get_vacancies_with_keyword(self, word):
        """ Функция возвращает список всех вакансий, в названии которых содержатся ключевое слово, например python"""
        self.conn = psycopg2.connect(**self.params)
        self.conn.autocommit = True
        self.cur = self.conn.cursor()
        self.cur.execute(f"""select vacancy_title,company_name,vacancy_salary,vacancy_description from 
        head_hunter_vacancies where vacancy_title like '%{word}%'""")
        rows = self.cur.fetchall()
        new_list = []
        if rows:
            print("По вашему запросу нашлись следующие вакансии с указанием работодателя, зарплатой и описанием")
